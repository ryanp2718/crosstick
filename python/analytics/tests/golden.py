...
def _trade(ex: str, sym: str, tid: str, px: str, sz: str, side: Side) -> MsgFactory:
    return lambda ex_ts, lo_ts: Trade(
        exchange=ex, symbol=sym, trade_id=tid, price=px, size=sz, side=side,
        exchange_ts_ns=ex_ts, local_ts_ns=lo_ts,
    )
...
def build_golden_records() -> list[CorpusRecord]:
    ...
    log.md(
        book_snapshot_topic(cb, s_cb), cb, s_cb,
        _snap(cb, s_cb, 100,
    ...
    log.md(
        book_delta_topic(cb, s_cb), cb, s_cb,
    ...
    log.md(
        book_snapshot_topic(kr, s_kr), kr, s_kr,
    ...
    log.md(
        book_snapshot_topic(bn, s_bn), bn, s_bn,
